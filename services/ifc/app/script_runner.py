...
def _runtime_ro_binds() -> List[str]:
    ...
    venv（sys.prefix，site-packages 里的 ifcopenshell）与 uv/pyenv 管理的
    解释器（sys.base_prefix）常在 /usr 之外，必须显式挂载。按挂载目标路径
    字面去重（**不能**按 realpath：/lib64→usr/lib 这类符号链接若按 realpath
...
def _run_in_sandbox(
    ...
    with tempfile.TemporaryDirectory(prefix="aiifc-run-") as workdir:
        ...
        cmd = _sandbox_cmd(settings, workdir) + [sys.executable, script_path, tmp_out]
