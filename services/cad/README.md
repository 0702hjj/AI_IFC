...
Chunk 边界——**本服务尚未包含**（后续 chunk 按 spec 2026-08-12-services-cad-script-as-source-design.md 工作项 3-7）：`script/locate`（handle→调用点定位）、`script/edit-call`（libcst 标量改写）、实体级语义 diff（`/models/{id}/diff`）、Go server 代理映射、`render.json` 渲染产物。
