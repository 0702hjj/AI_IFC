...
- **bwrap backend** (preferred, auto-detected): read-only root bind +
  writable sandbox cwd + ``--unshare-net``.
- **rlimit fallback** (no bwrap): same rlimits + isolated cwd/TMPDIR; FS
  ...
  that layer).
- both backends: ``RLIMIT_CPU``/``RLIMIT_AS``/``RLIMIT_NPROC`` + wall-clock
  timeout with process-group kill, stderr tail (2KB) on failure → 422.
...
def run_script(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="aidxf-run-") as workdir:
        ...
        cmd = _sandbox_cmd(workdir) + [sys.executable, runner_path, script_path, tmp_out]
