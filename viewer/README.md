...
- **属性面板**：Pset 分组折叠、属性搜索、属性复制（只读）
- **Issue/Markup**：选中构件创建 Issue（自动保存相机视角 + 截图）、状态流转（Open/Checking/Resolved）、点击恢复视角、删除
- **工具**：剖切（X/Y/Z 滑杆）、距离测量
...
- **server**（Go，stdlib only）：HTTP API 服务（默认端口 `:8090`），负责模型上传、转换任务调度、XKT/元数据静态服务、模型管理与 Issue CRUD。
...
web (React/xeokit)  ──HTTP──>  server (Go :8090)  ──子进程──>  converter (Node)
                                      ...
                                      └── data/  (上传的 IFC、生成的 XKT 与元数据、
                                                  issues.json + issues/*.png 审查标记)
```

Issue 持久化当前为文件存储（`internal/issue.Store` 接口抽象），后期可平移 PostgreSQL（新增 PgStore 实现，API/前端零改动）。
...
端到端冒烟（覆盖上传→转换→下载→Issue CRUD 全链路，需 server 已运行）：
...
cd server && go test ./...        # 后端 24 个测试
cd web && npm test                # 前端 47 个测试（另：npm run build 类型检查）
```
...
├── server/           后端 API 服务（Go）
│   ├── cmd/server/     服务入口
│   ├── internal/issue/ Issue 存储（Store 接口 + 文件实现）
│   └── server_config.json
├── web/              前端应用（React + xeokit + Vite）
├── scripts/smoke.sh  端到端冒烟脚本（含 Issue CRUD）
├── docs/             设计文档、API 文档、计划
└── data/             运行时数据（不纳入版本管理）
```
