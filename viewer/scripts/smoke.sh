...
if curl -sf "$EDIT_URL/health" > /dev/null 2>&1; then
  GUID=$(curl -sf "$BASE/v1/models/$ID/metadata.json" | python3 -c 'import sys,json;d=json.load(sys.stdin);print(next(o["id"] for o in d["metaObjects"] if o["type"]=="IfcWall"))')
  echo "edit target: $GUID"
  curl -sf -X PUT -H 'Content-Type: application/json' \
    -d '{"fields":{"Name":"Smoke Renamed Wall"}}' \
    "$BASE/api/v1/models/$ID/edit/entities/$GUID" > /dev/null
  curl -sf "$BASE/api/v1/models/$ID/edit/pending" | python3 -c 'import sys,json;d=json.load(sys.stdin)["data"];assert len(d)==1 and d[0]["changes"][0]["newValue"]=="Smoke Renamed Wall",d'
  curl -sf -X POST "$BASE/api/v1/models/$ID/edit/commit" \
...
assert any(e["operation"]=="update" and e["field"]=="Name" and e["newValue"]=="Smoke Renamed Wall" and e["diff"] for e in d),d'
  ...
  curl -sf -X POST -H 'Content-Type: application/json' -d '{"base":"v1","target":"current"}' \
    "$BASE/api/v1/models/$ID/edit/diff" | python3 -c "import sys,json;d=json.load(sys.stdin)['data'];assert any(c['guid']=='$GUID' for c in d['changed']),d"
  echo "edit flow OK"
else
