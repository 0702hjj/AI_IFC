...
- IFC 版本 delta 压缩方案（大版本锚点 + 字典压缩增量链）**正式归档为"不需要"**：版本 diff 全面复用 M5 已建的 script diff（text + PARAMS 键级，大/小版本两级），IFC 快照只是成对缓存。
...
前端路线（无 AI）：L2 的「选中 → 定位 → 表单改」建立在 script-backed 之上；plain 态模型前端不提供编辑入口（直改扬弃后的自然结果）。
