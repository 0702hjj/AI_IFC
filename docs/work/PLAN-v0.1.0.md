...
| v0.6 | services/cad script-as-source chunk D（分支 feat/v0.8-dxf-viewer）| W-0041 | kind=dxf 模型 ViewerPage 分流 Canvas 只读查看器（七类实体/pan-zoom/图层/选中属性/unsupported 明面化）测试绿；编辑 UI 另立 chunk 且开工需用户确认 |
