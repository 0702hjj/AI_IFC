...
| D13-D15 | **交付对齐（2026-08-20 裁决）**：D13 辅助产物挂载=方案③（plan/bim_supplement **方案级目录** `{DATA}/plans/{projectID}/` + 方案级版本化；building.json 版本级 sidecar，P-2 内容源=服务端读 CLI 文件）；D14 CAD 交付主链路=交付脚本 + run_script（create_project **空白化**——通用项目容器，无 kind 语义，从空白创建）；D15 前端新建表单（项目入口 + 会话绑 projectId）。设计全文见 `agent_delivery_alignment.md` | — |
