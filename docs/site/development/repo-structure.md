...
│   └── aidxfv/               #   CAD 生成/修改（ezdxf + vendored cadpy/archdxf；由 AI_CAD/skills 渐进收敛）
├── services/                 # ② 业务逻辑核心（diff + 面向前端修改的接口协议）
│   ├── ifc/                  #   IFC 段（现为 viewer/edit-service/，目录名历史别名）
│   └── cad/                  #   CAD 段（待建，与 ifc 同构）
├── viewer/                   # ③ 共享可选运行时
│   ├── web/                  #   前端（可选实现；React 19 + xeokit，:5173）
│   ├── server/               #   Go 网关（:8090，REST 入口 + 编排 + 存储抽象）
│   ├── converter/            #   Node 转换器（IFC → XKT）
│   ├── edit-service/         #   services/ifc 的物理实现（Python :8100）
│   ├── mcp-server/           #   MCP 桥（可选）
│   ├── scripts/smoke.sh      #   端到端冒烟
│   └── data/                 #   运行时数据（gitignored）
├── AI_CAD/                   # CAD skill 域 + 调研（skill 收敛进 skills/aidxfv，research 保留）
├── docs/
...
映射（渐进——存量路径不动，文档用目标名指代）：`viewer/edit-service/` 即 `services/ifc/`；`AI_CAD/skills/aidxfv*` 收敛进 `skills/aidxfv/`；`viewer/web|server|converter` 即共享可选运行时。SCAD 遗产代码（`src/simplecadapi/`、`skills/simplecadapi/`、根打包配置）已于 2026-08-06 移至私有归档仓 [0702hjj/SimpleCADAPI-archive](https://github.com/0702hjj/SimpleCADAPI-archive)，本仓不再包含。
