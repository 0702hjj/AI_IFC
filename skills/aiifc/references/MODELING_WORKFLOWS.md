...
## Design JSON Framing (frame + parameterize)

> For **complex floor plans** (multi-room partition runs), **irregular buildings** (sloped/curved walls), or **multi-storey**: the LLM emits a **design JSON** (geometric intent, no coordinate math) instead of improvising coordinates in code — eliminating drift/misalignment/gaps.

### When to use

- Complex / irregular / multi-storey → design JSON framing
- Simple single wall/slab → direct code (Pipeline Stages above)

### Three-layer method

```
① Frame:     LLM emits design JSON (whole frame + per-floor geometry intent)
