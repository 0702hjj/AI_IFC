...
## 9. 增补:A3 脚手架模板(2026-08-05)

- 新增 `references/scaffold_floor_plan.py`:DECLARATION ZONE(墙/开洞/房间/洁具声明,唯一编辑区)+ ASSEMBLY ZONE(固化组装机械,禁改)两段式模板;
- 验收:可跑、canon 后字节级重现、渲染六项结构正确、mm 图面积后缀 M2 修正;
- 接线:SKILL.md 建筑段第 4 步"起步必须复制模板"、floor_plan_assembly.md 头部指引——模型以改模板起步而非空白起步,收窄越界面。
