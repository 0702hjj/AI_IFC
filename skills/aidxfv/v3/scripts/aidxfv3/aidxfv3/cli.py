...
def build_parser() -> argparse.ArgumentParser:
    ...
    commands = {
        ...
        "draw": "archdxf 逐构件画法封装（worker 调用，非渲染器）",
        "svg": "DXF → SVG 导出",
        "readback": "底稿翻译：DXF → 房间图",
        "reconcile": "声明 vs 底稿对账",
        "sync": "DXF 直接编辑回收：哈希 → 回读 → audit → 更新声明",
        "pack": "mission 渲染（zone 切片 + 骨架段 + feedback → worker prompt）",
...
def _cmd_pack(args) -> int:
    ...
    if args.db and args.type:
        try:
            ...
            pass  # 无 db/无命中 → 不注入，worker 可 pull
...
def _cmd_draw(args) -> int:
    """draw 是 worker 逐构件原语（非渲染器）——CLI 入口验证包可用。"""
    import dxfkit.draw  # noqa: F401  lazy import，仅验证可导入
    _emit({"draw": "worker 直接 import dxfkit.draw 逐构件调用"}, None)
    return 0
