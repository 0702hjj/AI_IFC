"""dxfkit/draw.py —— archdxf 画法工具箱封装（worker 逐构件调用）。

设计纪律（mission_parrallel §3A）：
- **不是 JSON→DXF 渲染器**——每函数 = 一个构件，worker 逐构件调用；
- 每函数返回实体 key（key 身份映射落盘，geo_cognition §2）；
- 墙体用墙线（非 HATCH）——由 worker 传入墙轴线 + 厚，封装 wall_run/jamb/door。
...
class _AsciiDrawing(ezdxf.document.Drawing):
    ...
    用法：new_doc() 返回的即此子类，worker 照旧 doc.saveas("floor.dxf")。
    """
...
def draw_partition_base(msp, geom: dict) -> dict:
    ...
    底座 = 分区底图——房间/墙/门窗由 rooms-worker 增量画。
