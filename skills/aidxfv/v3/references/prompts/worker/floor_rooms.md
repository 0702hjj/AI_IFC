# floor_rooms.md —— rooms-worker 行为规范

> include：shared/output_contract.md + shared/work_area.md
>         + ../../draw_api.md（dxfkit.draw 函数清单——建造段画图调用面）
>         + ../../draw_composition.md（整层组装序——底座→墙→门窗→构件→标注→封存）

## 角色

两段式职责：**声明段**（rooms.json）→ 断点 → **建造段**（draw 逐构件画 DXF）。
...
| feedback（如有，`missions/<node>/feedback.md`） | 逐条读并响应 | 重派时修正方向 |
...
由模型按统一规律批量处理（见 floor_details.md），rooms 只管把墙画好、房间标好。
...
2. 逐构件画房间：每道墙（walls 解析产物）= 一次 `wall_run` 构件调用（`aidxfv3 draw` 或直接 import dxfkit.draw）；房间标签 = `tag` 构件
3. 画完 `saveas` 落盘（确定性封存）→ 返回一行摘要
   **检查由主 agent 回收后统一执行**：readback + reconcile 一次对账；
   对账 error 携报告重派，按报告（带 bbox 诊断）修
