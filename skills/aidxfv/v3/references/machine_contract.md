...
- `state` 负责状态记录；subagent 派发/回收由主 agent 用原生 agent 协议（dispatch.md）。
  状态推进规则（产物驱动）：rooms.json→declared；+geom.json→presented；
  +floor.dxf→built；+readback.json+geom_check.json(PASS)→done。
- 多 zone（异楼层裙房/塔楼）：`floors.json#dag.edges` 恒空，各 zone 独立 mission 并行——
  `state sync` 补出全部 mission，再逐个 `task` 并行派发。
